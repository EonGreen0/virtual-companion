...
## 十一、已确认的技术决策（2026-08-08 更新）

### 语音方案（角色无关）

- **TTS 选型**：豆包 TTS（火山引擎）预设音色，从几十种音色中试听选择，不进行声音克隆
- **情绪与语气词控制**：在 DeepSeek → 豆包 TTS 之间加文本加工层
  1. 提示词层：DeepSeek 生成口语化、适合朗读、无 markdown 的文本
  2. 加工层：剥离括号动作、控制语音长度（5~20 秒）、输出 `<emotion>` 情绪标签
  3. TTS 层：按情绪标签映射豆包 TTS 语气/语速参数
- **微信语音链路**：DeepSeek 文本 → 加工 → 豆包 TTS → 转 silk → iLink 发送语音消息
- **语音输入**：iLink 语音消息自带转文字，无需额外 ASR
- **节奏拟人**：回复前模拟 2~4 秒准备时间；按句分包，可连发 2~3 条短语音

### 角色定位

- 喜多郁代仅是示例角色，项目与人格卡、音色、记忆均角色无关，可整体替换
